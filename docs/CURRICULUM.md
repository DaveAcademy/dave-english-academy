...
- Curriculum gating (which lessons/vocabulary/game content a student can access) is computed server-side, unlock-number-based, and reused identically across the lesson hub, vocabulary games, and content-bank games — see `student_unlocked_lesson_number()` in `DATABASE.md`. As of the 2026-08-18 update, no active student is unlocked past Lesson 50 (current `curriculum_progress.max_available_lesson`: C=50, B=40, A=15, A1=10) — relevant context for why some game-level content bands (gated at Lesson 86) aren't yet reachable by any real student (**expected, not a bug** — see `GAMING-SYSTEM.md`).
- **Two independent gates, confirmed-current architecture (not new):** `curriculum_progress.max_available_lesson` is the *permanent curriculum ceiling* per level (set by migration/operator only, no UI writes it); `curriculum_progress.current_lesson_number` is the *day-to-day teacher-set schedule pace* (written from the Lessons page UI, `src/pages/Lessons.jsx` → `handleProgressSave`). `can_read_lesson_pdf()`/`student_unlocked_lesson_number()` require both to pass (ceiling AND pace) — this is the exact mechanism `0095` built and `0160` preserved; nothing new was added for the 2026-08-18 cap change, only the ceiling values.
...
- **Cap discrepancy from `0160`, resolved 2026-08-18 (migration `0161`):** `0160` found production's `max_available_lesson` (A=20, A1=10, B=40, C=50) didn't match an earlier boundary spec and correctly refused to change it, since Level A's pace was already at 14. Dave then gave new target boundaries (A=15, A1=10, B=40, C=50) explicitly as permanent ceilings, separate from pace. Safety check per level before applying: highest lesson any active student has ever legitimately completed was A=14, A1=5, B=40, C=50 (all ≤ target). Only **Level A's cap changed, 20→15** (`0161_level_a_permanent_cap_15`) — A1/B/C targets already equalled production, so no migration was needed for those. Verified live: `max_available_lesson` is now A=15/A1=10/B=40/C=50; a real Level A student (pace 14) can open lesson 14, cannot open lesson 15 (pace-gated, expected `0160` behavior) or lesson 16+ (now ceiling-gated, new). Lessons 16-20 were confirmed unused headroom — no group's pace or completion history ever reached them.
