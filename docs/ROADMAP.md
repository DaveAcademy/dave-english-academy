...
2. ~~**Game ranking improvement**~~ — **SHIPPED 2026-08-17**: additive "highest level reached" leaderboard view (Q9). Backend RPC (`0151`) was already live in prod with no frontend caller; wired up this session (`GameCenter.jsx`/`GameCard.jsx`). Existing score-based Top 5/rank untouched. Real-student UI verification still open (no test credentials available).
3. ~~**Achievement schema reconciliation**~~ — **already done**: `0105_achievement_engine_schema.sql` is the committed, numbered migration, confirmed applied to prod. `PROD_RECONCILED_achievement_engine_schema.sql` (untracked) is an earlier, superseded draft of the same reconciliation — both are idempotent, safe to leave as-is per 0105's own header. Stale roadmap item, not actioned further.
