...
- Level-scoped RLS extension to remaining role-only-gated tables — `curriculum_progress` done in `0156` (written 2026-08-17, not yet applied to production); `files` deferred pending a product decision on whether shared resources should be level-restricted at all. Low urgency, latent risk only.
