...
- **8 functions with mutable `search_path`** — found 2026-08-17 via Supabase's own `get_advisors(type: "security")` lint ("Function Search Path Mutable", WARN): `game_level_to_length_cap`, `game_level_to_tier`, `game_tier_bonus`, `month_bounds`, `week_bounds`, `get_activity_feed`, `get_monthly_payment_collection`, `get_payment_collection_summary`. None are `SECURITY DEFINER` (all run as invoker), but an unpinned `search_path` still lets a caller-controlled `search_path` shadow the unqualified `is_admin()` call inside the three admin-report functions with an object from another schema. Fix written, not a behavior change: `0157_function_search_path_hardening.sql` pins `search_path = public, pg_temp` on all 8 (all 8 bodies already schema-qualify their own table refs with `public.`, so this is a pure hardening no-op for current behavior). **Not yet applied to production** — `apply_migration` was denied at the tool layer this session (same standing production-write restriction documented in §5.1); needs a session/channel with DDL-apply permission, or Dave applying it directly.
