...
- **Teacher scoping:** `is_teacher()` alone (role-only, no level/group scope) was the pattern on ~15 tables until the 2026-08-14 remediation (migrations 0131-0136) added level-based scoping via `teacher_group_assignments` to the 6 highest-sensitivity tables (exam_scores, homework, lesson_work, student_lesson_progress, attendance, certificates). **known-issue-open (latent):** the remaining tables outside that batch (e.g. `files`, `curriculum_progress`) were deliberately deferred — real exposure is low (only 2 teachers exist, both all-level, at the time of the audit) but the gap is architecturally real.
