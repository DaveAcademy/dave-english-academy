...
export const DailyMissions = () => {
  ...
  useEffect(() => {
    ...
    loadProgress();
    const interval = setInterval(loadProgress, 300000); // refresh every 5 min
  ...
  if (!progress) {
    return (
      <div className="p-4 text-sm text-ink/60">
        {t('missions:noMissions', { defaultValue: 'No daily missions available at this time' })}
      </div>
    );
  }
    ...
    <div className="space-y-1">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-ink/10">
        <h3 className="font-medium text-ink">
          {t('missions:today', { defaultValue: 'Today\'s Missions' })}
        </h3>
        <span className="text-xs text-ink/60">
          {t('missions:reset', { defaultValue: 'Reset daily at Tashkent midnight' })}
        </span>
      </div>

      {/* Mission cards grid - 1 column mobile, 2-3 desktop */}
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {progress.missions.map((mission) => (
          <div
            key={mission.key}
            className="rounded-lg border border-ink/10 bg-white shadow-sm transition-all hover:shadow-md focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-500 focus-visible:outline-offset-2 reduced-motion:hover:shadow-none"
            onClick={() => window.location.href = '/my-daily-missions'}
          >
            <div className="p-3">
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <progress
                    className="w-24 h-2 rounded-md mt-1 bg-ink/5 border border-ink/20"
            ...
            {mission.completed && !mission.claimed ? (
              <button
                ...
                className="mt-2 w-full rounded-md py-1.5 text-sm font-medium bg-brand-600 text-paper hover:bg-brand-500 focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-500 focus-visible:outline-offset-2 transition-colors disabled:opacity-50 disabled:pointer-events-none"
      ...
      {progress.almostThere && progress.almostThere.length > 0 && (
        <div className="p-3 text-sm text-brand-600 rounded-lg animate-fade-in">
          {t('missions:almostThere', {
            defaultValue: 'Almost there! {{count}} more to complete a mission',
