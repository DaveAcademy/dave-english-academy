    ...
    <div>
      ...
      <section className="rounded-xl bg-white p-5 shadow-card">
        ...
        <p className="mb-3 text-sm text-ink/60">
          Right now, everything is saved directly in this browser (called "local storage"). That means:
        </p>
        <ul className="mb-4 list-inside list-disc space-y-1 text-sm text-ink/60">
          <li>Your data stays private to this device and this browser</li>
          <li>It's available offline, even with no signal</li>
          <li>It won't automatically sync if you open the app on a different phone or computer</li>
          <li>Clearing your browser's site data would erase it — so keep backups!</li>
        </ul>
        <div className="rounded-lg bg-brand-50 p-3 text-sm text-brand-700">
          <div className="mb-1 flex items-center gap-1.5 font-semibold">
            <Info size={14} /> Upgrading to cloud storage later
          </div>
          <p className="text-brand-700/90">
            The app is built so this can be upgraded to a real online database (like Supabase) without changing how
            any screen works — only one file (<span className="font-mono text-xs">src/lib/db.js</span>) would need to
            be rewritten. See <span className="font-mono text-xs">SUPABASE_UPGRADE.md</span> in the project for a
            step-by-step guide whenever you're ready for that.
          </p>
        </div>
      </section>
    </div>
