  ...
  return (
    <div>
      <header className="mb-6">
        <h1 className="font-display text-2xl font-bold text-ink">My Homework</h1>
        <p className="mt-1 text-sm text-ink/50">Download assignments, submit your work, and see feedback.</p>
      </header>
              ...
              <div key={h.id} className="rounded-xl bg-white p-3 shadow-card">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-ink/50">Due {h.due_date} · {status.status}</p>
                  </div>
                  {graded && status.score != null && <p className="flex-shrink-0 text-sm font-bold text-brand-500">{status.score} / 100</p>}
                </div>
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  {h.file_url && (
                    ...
                    >
                      <Download size={13} /> {h.file_name || 'Homework file'}
                    </button>
                  )}
                  {!graded && (
                    <label className="flex cursor-pointer items-center gap-1.5 rounded-lg border border-ink/10 px-3 py-1.5 text-xs font-semibold text-ink/60 hover:bg-ink/5">
                      <Upload size={13} />
                      {submittingId === h.id ? 'Uploading...' : status.answer_file_name ? 'Replace my submission' : 'Submit my work'}
                      <input
                        type="file"
