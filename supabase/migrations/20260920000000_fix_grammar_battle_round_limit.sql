-- Fix: get_grammar_battle_round() returns only 1 word (limit 1)
-- The Grammar Battle component expects a pool of words (like other games return 10+)
-- With limit 1, the pool exhausts after 1 question, causing "No vocabulary available"
-- Change limit to 10 to match the pattern of other game round RPCs
--
-- This ensures the round has enough vocabulary for the tier-based selection
-- (very_easy -> easy -> medium -> hard -> very_hard fallback logic)
-- to work correctly, preventing the empty-pool error on mobile.
...
begin
  ...
  order by
    case
      ...
      when (payload->'difficulty') = 'hard' then 4
      when (payload->'difficulty') = 'very_hard' then 5
    end,
    random()
  limit 10;
end;
