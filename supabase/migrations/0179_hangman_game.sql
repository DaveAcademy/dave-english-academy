-- Hangman: replaces Listening Challenge in the visible Game Center
-- (2026-08-19 - on-device TTS proved unreliable on Android, no code fix
-- possible; see GameCenter.jsx comment). Pure text/tap game, no audio,
-- no images - can't hit the same device-dependency problem.
--
-- Family V game, same shape as Word Scramble/Word Builder: content comes
-- from pick_game_words(true, 8, level) (alpha-only, level-driven length
-- cap), client does classic letter-by-letter guessing with a wrong-guess
-- limit, grading happens in submit_game_round() same as every other
-- vocab-graded game (lower(trim(answer)) = lower(english)). Also returns
-- uzbek so the client can show a translation hint (letters alone with no
-- context is unnecessarily hard).
...
begin
  ...
  for r in select p.id, p.english, p.uzbek from public.pick_game_words(true, 8, v_level) p
  loop
...
-- Byte-for-byte identical to the live definition except: 'hangman' added
-- to the allowlist, metric-key map, and the vocab-graded branch's game
-- list/comparison (same rule as word_scramble/word_builder - case-
-- insensitive, trimmed, full-word match).
