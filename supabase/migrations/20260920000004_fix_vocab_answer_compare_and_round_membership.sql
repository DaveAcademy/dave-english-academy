-- POST-FIX FORENSIC AUDIT: 2026-09-08.
--
-- Three defects found by the forensic audit against the live function and the
-- real clients (this migration is a CREATE OR REPLACE of the 3-arg
-- submit_game_round, keeping the full+perfect advancement gate from 0003):
--
-- 1. (CRITICAL) Vocabulary-branch grading compares EVERY answer against
--    lv.english, but the real clients submit Uzbek text for three games:
--      VocabularyQuiz.jsx  -> answer = selected Uzbek option
--      WordMatch.jsx       -> answer = tile.text (Uzbek)
--      SpeedChallenge.jsx  -> answer = selected Uzbek option
--    Admissions/translations prove `lv.english` never equals the Uzbek answer,
--    so vocabulary_quiz / word_match / speed_challenge could never score a
--    single point in the real app. 0150 graded those three against
--    lesson_vocabulary.uzbek; the 20260906000000 rewrite flattened it to
--    english-only and the subsequent "fix" series kept that regression.
--    Reproduced live (student 86, sessions 7428/7429/7430): real client
--    payloads score 0/10 while english-fed harness payloads score 100.
--    Fix: restore per-game comparison (english for word_scramble /
--    word_builder / hangman; uzbek for vocabulary_quiz / word_match /
--    speed_challenge), matching the 0150 canonical contract.
--
-- 2. (SECURITY/PROGRESSION) The submitted vocabulary_id / content_id values
--    were never checked against the round's stored id set. A client could
--    submit fabricated/deduplicated/overlapping answers that are individually
--    correct and still hit the full+perfect gate (e.g. the same 10 words
--    repeated, or words from a different round). The 0003 gate only checks
--    word COUNT equality, not id membership. Fix: require every submitted id
--    to be a member of the round's vocabulary_ids and reject duplicates and
--    id-substitution so a real full+perfect round of the ACTUAL round words
--    is the only way through the gate.
--
-- 3. (word_match perfect bonus) v_wrong_attempts was only populated in the
--    content-game branch, so for word_match (a vocabulary-branch game) it
--    stayed NULL and coalesce(NULL, 0) = 0 made the +5 perfect bonus ALWAYS
--    granted on level_up. Fix: read wrong_attempts in the vocabulary branch
--    so the perfect bonus requires a genuinely mistake-free round, matching
--    the intended rule.
--
-- Idempotent: CREATE OR REPLACE. Scoring per correct (10 pts, 5 with hint,
-- speed bonus) is unchanged. Full+perfect advancement gate unchanged.
