-- Fix: Picture Quiz not handled in submit_game_round (migration 0196)
--
-- The 0196 migration redesigned submit_game_round with two branches:
--   1. Vocabulary games (vocabulary_id based): word_scramble, vocabulary_quiz, word_match, speed_challenge, word_builder, listening_challenge, hangman
--   2. Curated content games (content_id based): sentence_scramble, word_detective, grammar_battle
--
-- Picture Quiz (content_id based, uses game_content_bank.payload->>'english') was accidentally
-- omitted from BOTH branches, so it falls into the else branch but without a specific
-- matching condition, causing incorrect grading (always v_correct = false).
--
-- Fix: Add 'picture_quiz' to the curated content games list in the condition check,
-- and ensure the answer grading logic handles it correctly.
...
begin
  ...
  -- Curated content games (content_id based, uses game_content_bank)
  else
    for r in
      select
