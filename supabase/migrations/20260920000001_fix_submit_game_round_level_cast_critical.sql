-- HOTFIX (critical): submit_game_round() throws on every submission.
--
-- ROOT CAUSE: live submit_game_round() (reintroduced byte-for-byte by
-- 20260906000000_fix_game_whitelist_replay_tier.sql, commit 61c5f7a) runs
--
--     select id, level into v_student_id, v_words_correct
--       from public.students where profile_id = auth.uid();
--
-- students.level is TEXT ('A1'/'A'/'B'/'C'), but v_words_correct is INTEGER,
-- so Postgres casts the level text to integer and raises:
-- "invalid input syntax for type integer: \"C\"" (or 'A'/'B'/'A1').
-- This fires on the very first line of the function body, so EVERY round
-- submission fails: the final question never grades, game_sessions and
-- game_points_transactions are never created, Game Points are never awarded,
-- and the result screen never appears. Games mint rounds fine ( = 80 rounds
-- in the last 24h) but nothing past the cast is reached (0 sessions, 0 points
-- in the last 24h; last session 2026-09-04 15:31:59 UTC).
--
-- This was already fixed once by 20260828002000 (commit 3b42a1e) but the
-- 20260906000000 rewrite reverted it. The 'level' column is not used anywhere
-- else in this function (the very next line resets v_words_correct := 0), so
-- selecting only students.id is safe.
--
-- FIX (smallest possible): drop the 'level' read from the SELECT. Nothing
-- else changes: same 10-game whitelist, same replay protection, same grading
-- (10 pts per correct / 5 with hint / speed bonus), same 10-of-10 tier
-- advancement, same points ledger, same RLS/grants.
--
-- 2026-09-20 (applied directly to production via supabase db query --linked
-- --file, NOT via db push, because db push would also apply the orphaned
-- local-only migration 0209_admin_batch_payment_status.sql which is out of
-- scope). Idempotent: CREATE OR REPLACE.
...
begin
  ...
  if v_words_correct = 10 and v_words_total = 10 and v_round_level is not null then
