-- Game Points Redesign: per-game difficulty tiers with clear point values.
--
-- Problem: the current formula (10 + tier_bonus) produces confusingly small
-- values (10-27) that don't match the displayed "score" (word_correct * 10
-- = up to 100). Students see "100" on the results screen but earn only
-- 15-22 Game Points, creating a trust gap.
--
-- Solution: each game has a fixed difficulty tier that determines its point
-- value directly. The level still controls CONTENT difficulty (which words
-- appear), but the point reward is based on the game's inherent complexity.
--
-- New tier point values (base per round, BEFORE perfect bonus):
--   very_easy:   5 points
--   easy:       10 points
--   medium:     20 points
--   medium_hard: 30 points
--   hard:       40 points
--   very_hard:  50 points
--
-- Perfect bonus: +5 points (unchanged, only on leveled_up with 100% correct)
--
-- The game_type_difficulty() function maps each game to its fixed tier.
-- submit_game_round() uses this instead of game_level_to_tier() for points.

-- =====================================================================
-- 1. Per-game difficulty tier (fixed, not level-dependent)
-- =====================================================================
...
as $$
  select case p_game_type
    when 'picture_quiz'       then 'very_easy'    -- 5 pts: visual recognition, 4 options
    when 'hangman'            then 'easy'         -- 10 pts: letter guessing, hint available
    when 'vocabulary_quiz'    then 'easy'         -- 10 pts: MCQ recognition, 4 options
    when 'word_match'         then 'medium'       -- 20 pts: active recall, pair matching
    when 'word_scramble'      then 'medium'       -- 20 pts: spelling, unscramble
    when 'sentence_scramble'  then 'medium_hard'  -- 30 pts: sentence construction
    when 'word_builder'       then 'medium_hard'  -- 30 pts: letter assembly, spelling
    when 'word_detective'     then 'hard'         -- 40 pts: error identification + correction
    when 'speed_challenge'    then 'hard'         -- 40 pts: timed, speed pressure
    when 'grammar_battle'     then 'very_hard'    -- 50 pts: timed, lives, escalating tiers
    else 'easy'
  end
$$;

comment on function public.game_type_difficulty(text) is
  'Fixed difficulty tier per game type. Determines Game Points awarded per round. Does NOT affect content difficulty (that is still level-driven via game_level_to_tier).';

-- =====================================================================
-- 2. Updated tier bonus: now returns the FULL base point value (not a
--    bonus on top of 10). Submit_game_round will use this directly.
-- =====================================================================
...
-- =====================================================================
-- 3. Add medium_hard tier to game_level_to_tier (for content selection)
-- =====================================================================
...
as $$
  select case
    when p_level <= 5  then 'very_easy'
    when p_level <= 15 then 'easy'
    when p_level <= 25 then 'medium'
    when p_level <= 40 then 'medium_hard'
    when p_level <= 60 then 'hard'
    else 'very_hard'
  end
$$;

-- =====================================================================
-- 4. Update submit_game_round: use game_type_difficulty for points,
--    not game_level_to_tier. Level still drives content selection.
-- =====================================================================
...
begin
  ...
  if p_game_type in ('word_scramble', 'vocabulary_quiz', 'word_match', 'speed_challenge', 'word_builder', 'listening_challenge', 'hangman') then
    ...
    loop
      ...
      -- track wrong_attempts for word_match perfect bonus
  ...
  -- CHANGED: tier now comes from game_type_difficulty() (fixed per game)
  -- instead of game_level_to_tier() (varies by level). Point values are
  -- now the tier's full value directly, not 10 + bonus.
  if v_leveled_up then
    v_tier := public.game_type_difficulty(p_game_type);
    v_is_perfect := v_words_total > 0 and v_words_correct = v_words_total;

    -- Word Match: zero wrong_attempts required for perfect bonus
