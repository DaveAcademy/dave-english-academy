-- Fix: Advancement gate for non-10-question game rounds.
--
-- Problem: submit_game_round() only triggers progression (level_up) when
--   v_words_correct = 10 and v_words_total = 10
-- which is hardcoded for the fixed 10-question games. The content games have
-- different, deliberate round sizes (sentence_scramble=6, word_detective=8,
-- grammar_battle=20) and so does word_builder (8). A PERFECT round in those
-- games never satisfies the 10-and-10 gate, so they stay at level 1 forever
-- and never earn tier points (game_points_transactions only fires on level_up).
--
-- Verified in production (E2E student 86): grammar_battle 20/20, word_detective
-- 8/8, sentence_scramble 6/6, word_builder 8/8 all completed as perfect rounds
-- but game_level_progress stayed current_level=1 with zero points transactions,
-- while every 10-question game advanced to level 2+ and earned tier points.
--
-- Fix: advance on a FULL, PERFECT round instead of the hardcoded 10-and-10.
-- Condition: the round was fully submitted (v_words_total = v_round_size) AND
-- every submitted answer was correct (v_words_correct = v_words_total).
-- - For the 10-question games this is identical to the old "perfect 10/10":
--   a player must submit all 10 and get all 10 right to advance.
-- - For the fixed-size non-10 games (sentence_scramble=6, word_detective=8,
--   word_builder=8, grammar_battle=20) a full + perfect round now advances,
--   which the old hardcoded 10-and-10 gate never allowed.
-- The v_words_total = v_round_size requirement blocks two exploits:
--   (a) submitting only a few correct answers (e.g. 1 correct -> "perfect")
--       to advance without playing the round;
--   (b) padding extra answers to inflate v_words_total.
-- A non-perfect OR incomplete round still never advances. The leveled_up
-- points block already computes v_is_perfect the same way, so this is
-- consistent with tier points.
--
-- Idempotent: CREATE OR REPLACE. Scoring per correct (10 pts, 5 with hint,
-- speed bonus) is unchanged. No other game logic touched.
