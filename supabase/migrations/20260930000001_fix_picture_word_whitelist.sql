-- FIX: picture_word was missing from submit_game_round() whitelist.
-- The Picture Word game (migration 20260928000000_picture_word.sql)
-- provides a full page, round generator (get_picture_word_round),
-- grading logic (in the else branch), and Game Center tile, but the
-- submit_game_round() game_type whitelist only listed 9 of the 10
-- active games. Every student who submitted a Picture Word round got
-- "Unknown game_type: picture_word". This is a silent failure - the
-- client shows a generic error, no points or progression are recorded.
--
-- Smallest safe fix: add 'picture_word' to the existing whitelist.
-- The grading logic for picture_word already exists in the else
-- branch (grading vs game_content_bank.payload->>'english'), so no
...
begin
  ...
  if not found then
    select result_payload, game_type
      into v_recovered_payload, v_recovered_type
      from public.game_rounds
     where id = p_round_id
       and student_id = v_student_id;

    if v_recovered_payload is not null then
      if v_recovered_type is distinct from p_game_type then
  ...
  if p_game_type in ('word_scramble', 'vocabulary_quiz', 'word_match', 'speed_challenge', 'word_builder', 'hangman') then
    ...
    loop
      ...
      if r.content_id is null or r.skipped then
        ...
        elsif p_game_type in ('picture_quiz', 'picture_word') then
  ...
  if v_words_total > 0 and v_round_level is not null then
    ...
    v_earned := v_words_correct * case p_game_type
        when 'picture_quiz'      then 1
        when 'vocabulary_quiz'   then 1
        when 'hangman'           then 1
        when 'picture_word'      then 1
        when 'word_match'        then 2
        when 'word_scramble'     then 2
        when 'word_builder'      then 3
        when 'sentence_scramble' then 3
        when 'speed_challenge'   then 4
        when 'grammar_battle'    then 5
        else 1
      end;
    ...
    v_banked_delta := greatest(coalesce(v_prior_points, 0), v_earned) - coalesce(v_prior_points, 0);

    if v_banked_delta > 0 then
      select level into v_academy_level
        from public.students
       where id = v_student_id;

      insert into public.point_transactions
        (student_id, level, category_id, category_key, points, reason, awarded_by)
      values
        (v_student_id, v_academy_level,
         (select id from public.point_categories where key = 'game_activity'),
         'game_activity', v_banked_delta,
         'Game points: ' || p_game_type || ' level ' || v_round_level,
         auth.uid());
    end if;

    v_points_awarded := v_banked_delta;
  end if;
