-- Game system upgrade completion (WordMatch fairness + difficulty scaling).
--
-- WordMatch faucet fix:
--   The matching UI only submits once every pair is matched correctly, so
--   the server could never observe a failed round: words_correct always
--   equalled words_total, is_perfect was always true, and the +5 perfect
--   bonus was meaningless. Combined with a fixed 6-pair round at every
--   tier, higher levels were not harder and grinding stayed trivial.
--
-- Changes:
--   1. get_word_match_round(): pair count now scales with tier
--      (very_easy 6 -> easy 7 -> medium 8 -> hard 9 -> very_hard 10),
--      mirroring the tier system every other game uses.
--   2. submit_game_round(): answers may carry an optional wrong_attempts
--      count (mismatched tap attempts). For word_match the +5 perfect
--      bonus now requires zero mismatched attempts - restoring its
--      meaning - while base scoring (10 + tier bonus) is unchanged.
--      Old clients that omit the field behave exactly as before.
...
begin
  ...
  -- Difficulty progression: more pairs per round as tiers rise, so later
  -- levels are meaningfully harder and pure grinding slows down.
  v_pair_count := case public.game_level_to_tier(v_level)
    when 'very_easy' then 6
    when 'easy' then 7
    when 'medium' then 8
    when 'hard' then 9
    else 10
  ...
  for r in select p.id, p.english, p.uzbek from public.pick_game_words(false, v_pair_count, v_level) p
  loop
...
begin
  ...
  if p_game_type in ('word_scramble', 'vocabulary_quiz', 'word_match', 'speed_challenge', 'word_builder', 'listening_challenge', 'hangman') then
    ...
    loop
      ...
      v_wrong_attempts := v_wrong_attempts + r.wrong_attempts;
  ...
  if v_leveled_up then
    ...
    -- WordMatch: the matching UI can always complete a round, so the +5
    -- perfect bonus only pays out when every pair was matched without any
    -- mismatched attempt (old clients omit wrong_attempts: old behavior).
    if p_game_type = 'word_match' then
      v_is_perfect := v_is_perfect and v_wrong_attempts = 0;
    end if;
