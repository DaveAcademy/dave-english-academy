-- HOTFIX: submit_game_round() grades Uzbek-answer games against English column.
--
-- ROOT CAUSE: live submit_game_round() compares student answers against
-- lesson_vocabulary.english for ALL vocabulary-based games. However, three
-- games present Uzbek options to the student and send the Uzbek answer:
...
-- The correct answer is the Uzbek text, but the live function compared it
-- against the English column -> correct answers marked wrong, English words
-- wrongly accepted. Introduced by the 20260827223000 "definitive" rewrite
-- which flattened the historically-correct per-game-type comparison into a
-- single english comparison. Historical migrations (0112, 0118, 0119, 0141,
-- 0142, 0150, 0152, 0155) all had:
...
--
-- FIX (smallest safe): restore per-game-type comparison in the shared
-- vocabulary-games branch. Nothing else changed: grading logic, points,
-- levels, pass threshold, RLS, grants untouched. Verified against production
-- in rolled-back transactions: vocabulary_quiz 2/2 correct Uzbek -> correct;
-- word_match/speed_challenge also fixed; English-word games (word_scramble,
-- word_builder, hangman) unchanged.
--
-- 2026-08-28

-- This migration replaces the live submit_game_round function with the
-- per-game-type comparison restored for the vocabulary-based games branch.
-- The function body is byte-identical to the live 20260827223000 version
-- except for the single comparison line.

-- First revoke existing grants
...
-- The complete function definition (CREATE OR REPLACE)
...
begin
  ...
  select id, level into v_student_id, v_words_correct from public.students where profile_id = auth.uid();
  v_words_correct := 0;
