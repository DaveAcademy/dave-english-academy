-- COMBINED HOTFIX: submit_game_round() - two independent bugs fixed together.
--
-- BUG 1: Vocabulary-answer games graded against wrong column.
-- ROOT CAUSE: live submit_game_round() compares student answers against
-- lesson_vocabulary.english for ALL vocabulary-based games. Three games
-- present Uzbek options and send Uzbek answers:
--   vocabulary_quiz  (pick Uzbek translation from 4 options)
--   word_match       (tap the Uzbek match for an English word)
--   speed_challenge  (pick Uzbek translation from 4 options, timed)
-- Live function compared Uzbek answers against English column -> correct
-- answers marked wrong, English words wrongly accepted. Introduced by the
-- 20260827223000 "definitive" rewrite which flattened historically-correct
-- per-game-type comparison into a single english comparison. Historical
-- migrations (0112, 0118, 0119, 0141, 0142, 0150, 0152, 0155) had:
...
--
-- BUG 2: Picture Quiz (and all games) crashed for every student.
-- ROOT CAUSE: live submit_game_round() selects students.level (TEXT: 'A1',
-- 'A', 'B', 'C') into the INTEGER variable v_words_correct:
--   select id, level into v_student_id, v_words_correct
--     from public.students where profile_id = auth.uid();
-- Postgres casts text level to integer -> "invalid input syntax for type
-- integer: "C"". This fires at function start, so EVERY round submission
-- fails. The very next line `v_words_correct := 0;` proves the `level`
-- read was unintended. Previous correct code (20260826112209, 0196) read
-- level into a TEXT variable `v_academy_level`; the "definitive" rewrite
-- dropped that variable and retargeted into integer v_words_correct.
--
-- FIX (smallest safe, both bugs in the same function):
-- 1. Restore per-game-type comparison in the vocabulary-games branch:
--    vocabulary_quiz, word_match, speed_challenge compare against uzbek;
--    word_scramble, word_builder, hangman keep english.
-- 2. Select only students.id into v_student_id; the level column is unused
--    (next line already resets v_words_correct := 0).
--
-- Nothing else changed: grading logic, points, levels, pass threshold,
-- RLS, grants untouched. Verified against production in rolled-back
-- transactions:
--   vocabulary_quiz 2/2 correct Uzbek -> correct;
--   word_match/speed_challenge also fixed;
--   English-word games (word_scramble, word_builder, hangman) unchanged;
--   Picture Quiz (and all games) no longer crash on submit.
--
-- 2026-08-28
