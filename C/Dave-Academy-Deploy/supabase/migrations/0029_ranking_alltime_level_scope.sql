-- Ranking v2, fix 1 of 1: get_group_leaderboard()'s and
-- get_student_ranking_summary()'s all_time branches ranked by
-- students.points, which the 0020 cache trigger sums across every level a
-- student has ever earned points in (no level filter) - inconsistent with
-- week/month, which both sum point_transactions scoped to pt.level =
-- p_level. A student who transfers levels would rank on points earned in
-- a level they're no longer competing in. No production student is
-- currently affected (verified: zero students have point_transactions
-- rows spanning more than one level as of this migration), so this is a
-- forward-safety fix, not a correction of any currently-displayed number.
--
-- Baseline rows stay included in the all_time sum, deliberately: this is
-- still each student's own real point history in that level, just not
-- broken out from ordinary awards the way week/month's `not
-- pt.is_baseline` filter breaks it out from period-recent activity.
-- get_student_ranking_summary()'s returned `lifetime_points` value is left
-- untouched (still the true cross-level students.points cache, used for
-- the student's own hero-stat display) - only its `level_rank_all_time`
-- ranking changes, to stay consistent with what get_group_leaderboard()
-- now ranks by.
--
